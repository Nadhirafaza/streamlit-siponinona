        ...
        **SIPONINONA (Sistem Informasi Pemetaan Kondisi Komponen Pengelolaan Sampah)** adalah platform digital berbasis data yang dirancang untuk:
        ...
        if st.session_state.df_clustered is not None:
           ...
           # Diagram Pie: Persentase tiap cluster
            ...
            df_map = st.session_state.df_clustered.merge(
                st.session_state.df,  # data asli sebelum normalisasi
                on='Nama Kecamatan',
                how='left',
                suffixes=('_clustered', '_original')
            ).merge(
                coords_df,
                on='Nama Kecamatan',
                how='left'
                ...
                if pd.notna(lat) and pd.notna(lng) and pd.notna(cluster_label):
                    folium.CircleMarker(
                        ...
                        popup=(
                            ...
                            f"Volume Sampah Tidak Terlayani: {row['Volume Sampah Tidak Terlayani_original']}<br>"
                            f"Jarak ke TPA: {row['Jarak ke TPA_original']} km<br>"
                            f"Jumlah Desa: {row['Jumlah Desa_original']}<br>"
                            f"Jumlah Penduduk: {row['Jumlah Penduduk_original']}"
                        )
                    ).add_to(m)
